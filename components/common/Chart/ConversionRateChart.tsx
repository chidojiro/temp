...
const ConversionRateChart = ({ firstColumn, secondColumn, lineChart, data }: ConversionRateChartProps) => {
    ...
    const { amchartsElRef } = useAmcharts(am4charts.XYChart, (chart) => {
      ...
      chart.events.on("beforedatavalidated", function() {
        for(let i = 0; i < chart.data.length; i++) {
          chart.data[i] = {...chart.data[i], year_month: `${chart.data[i].year}/${chart.data[i].month}`}
        }
      });
